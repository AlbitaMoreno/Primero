...
void Matriz2D_2 :: InicializaMismoValor(TipoBase valor){
...
void Matriz2D_2 :: ModificarValor (int fila, int col, TipoBase val){
	if(fila < filas){
		if(col < columnas)
			datos[fila][col] = val;
	}
...
TipoBase Matriz2D_2 :: LeerValor(int fila, int col)const{
	if(fila < filas){
		if(col < columnas)
...
void Matriz2D_2 :: Inicializa(const TipoBase valor){	//si no se le pasa un parámetro tiene un valor por defecto, el 0
	InicializaMismoValor(valor);
}
