/*Sobre el ejercicio anterior, construid un método que busque la fila de la matriz que
más se parezca a una secuencia de enteros, a la que llamaremos referencia. La similitud entre dos secuencias 
x = (x1    xp) e y = (y1    yp) 
vendrá dada por la distancia euclídea entre ambas:

dist(x; y) =q(x1 .. y1)2 +    + (xp .. yp)2

Además, la búsqueda solo se hará sobre las filas de la matriz enumeradas en una
segunda secuencia llamada filas_a_comparar.
Por ejemplo, dada la matrizM (7  4),
...
programa deberá encontrar 5 como la fila más cercana a referencia. En el dibujo
anterior se han marcado con una flecha las filas indicadas por filas_a_comparar.*/
...
class MatrizRectangularEnteros{
	...
	public:
		...
		int Euclides(MatrizRectangularEnteros matriz_privada, SecuenciaEnteros referencia_fila, SecuenciaEnteros filas){
			MatrizRectangularEnteros matriz(matriz_privada);
			SecuenciaEnteros referencia(referencia_fila);
			...
			for(int i = 0; i < filas_a_comparar.TotalUtilizados(); i++){
				for (int  j = 0; j < referencia.TotalUtilizados(); j++){
					...
					if (referencia.GetNumero(j) == matriz.GetElemento(filas_a_comparar.GetNumero(i), j)){
						coincidencias++;
					}
				}
				...
				if( coincidencias > mayor_coincidencias){
					...
					fila_coincidencia = filas_a_comparar.GetNumero(i);
...
int main(){
	...
	//busque la fila de la matriz que más se parezca a una secuencia de enteros
	SecuenciaEnteros referencia;
	SecuenciaEnteros filas_comparar;
	int digito, fila_a_comparar, mayor;
	bool filtro;
	
	cout << "\nIntroduzca una secuencia de enteros, para terminar introduzca '#'";
	do{
		cout << "\nIntroduzca un digito: ";
		cin >> digito;
		
		filtro = digito != '#';
		if(filtro){
			referencia.SetCoeficientes(digito);
		}
	}while(filtro);
	

	cout << "\n\Introduzca los numeros de fila para comparar: ";
	cout << "\nIntroduce una fila ('#' para terminar): ";
	...
	do{
		...
		filtro = digito != '#';
		if(filtro){
			filas_comparar.SetCoeficientes(fila_a_comparar);
		}
	}while(filtro);
	
	mayor = celda.Euclides(celda, referencia, filas_comparar);

	cout << "\n\nLa fila con mayor coincidencias es " << mayor;
